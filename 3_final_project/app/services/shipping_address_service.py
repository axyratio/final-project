...
def create_shipping_address_service(db, user_id, data):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Failed to create address: {str(e)}")
...
def update_shipping_address_service(db, ship_addr_id, data):
    try:
        ...
        rollback_and_cleanup(db, None)
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Failed to update: {str(e)}")
...
def delete_shipping_address_service(db, ship_addr_id):
    try:
        ...
        rollback_and_cleanup(db, None)
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Failed to delete: {str(e)}")
