...
//   },
// });
// app/(admin)/report.tsx — list only, ไม่มีรูป ไม่มีปุ่ม action
import { formatReportReason, formatReportStatus, getAllReports } from "@/api/report";
import { Ionicons } from "@expo/vector-icons";
import { useRouter } from "expo-router";
import React, { useEffect, useState } from "react";
import {
...
export default function AdminReportsScreen() {
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    let imageCount = 0;
    try {
      const urls = typeof item.image_urls === "string"
        ? JSON.parse(item.image_urls)
        : item.image_urls;
      imageCount = Array.isArray(urls) ? urls.length : 0;
    } catch {}

    return (
      <TouchableOpacity style={styles.card} onPress={() => handleCardPress(item)} activeOpacity={0.7}>
        {/* header: ไอคอน + ประเภท + badge สถานะ */}
        <View style={styles.cardHeader}>
          <Ionicons
            name={item.report_type === "user" ? "person-circle-outline" : "storefront-outline"}
            size={22}
            color="#ef4444"
          />
        ...
        {/* ข้อมูลสรุป */}
        <Text style={styles.reportedName}>{item.reported_name}</Text>
        <Text style={styles.reason}>เหตุผล: {formatReportReason(item.reason)}</Text>
        {item.description ? (
          <Text style={styles.description} numberOfLines={2}>{item.description}</Text>
        ) : null}

        {/* footer: ผู้รายงาน + จำนวนรูป + ลูกศร */}
          ...
          <View style={styles.footerRight}>
            {imageCount > 0 && (
              <View style={styles.imageCountBadge}>
                ...
                <Text style={styles.imageCountText}>{imageCount} รูป</Text>
              </View>
            )}
              ...
              return (
                ...
                >
                  {filterLoading && active && (
                    <ActivityIndicator size={11} color="#fff" style={{ marginRight: 4 }} />
                  )}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              ...
              <Text style={styles.emptyText}>ไม่พบรายงาน</Text>
            </View>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  filterLoadingText: { fontSize: 14, color: "#6b7280" },
  ...
  chip:            { flexDirection: "row", alignItems: "center", paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, backgroundColor: "#f3f4f6", borderWidth: 1, borderColor: "#e5e7eb" },
  ...
  cardType:     { fontSize: 14, fontWeight: "600", color: "#ef4444", flex: 1 },
  badge:        { paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  badgeText:    { fontSize: 12, fontWeight: "600" },
  reportedName: { fontSize: 16, fontWeight: "700", color: "#1f2937" },
  reason:       { fontSize: 14, color: "#6b7280" },
  description:  { fontSize: 13, color: "#9ca3af", fontStyle: "italic" },

  cardFooter:      { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 6, paddingTop: 10, borderTopWidth: 1, borderTopColor: "#f3f4f6" },
  reporter:        { fontSize: 13, color: "#9ca3af" },
  footerRight:     { flexDirection: "row", alignItems: "center", gap: 8 },
  imageCountBadge: { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "#f3f4f6", paddingHorizontal: 8, paddingVertical: 3, borderRadius: 10 },
